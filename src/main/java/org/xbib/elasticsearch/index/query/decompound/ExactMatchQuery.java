...
public class ExactMatchQuery {
    ...
    public ExactMatchQuery(QueryShardContext context) {
    ...
    public Query parse(Type type, String fieldName, Object value) throws IOException {
        ...
        ExactMatchQueryBuilder builder = new ExactMatchQueryBuilder(analyzer, fieldType);
    ...
    private class ExactMatchQueryBuilder extends QueryBuilder {
         ...
         */
        ExactMatchQueryBuilder(Analyzer analyzer, MappedFieldType mapper) {
            super(analyzer);
            this.mapper = mapper;
        }
    ...
    protected Query blendPhraseQuery(PhraseQuery query, MappedFieldType fieldType) {
    	SpanQuery[] clausesIn = new SpanQuery[query.getTerms().length];
    	int i = 0;
    	for(Term term: query.getTerms()) {
    		clausesIn[i] = new SpanPayloadCheckQuery(new SpanTermQuery(term), Collections.singletonList(null));
    		i++;
    	}
    	return new SpanNearQuery(clausesIn, query.getSlop(), query.getSlop() == 0);
    }
